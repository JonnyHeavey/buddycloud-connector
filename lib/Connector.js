    ...
    if(plugin.on) {
        plugin.on('messageReceived', function (data) {
            self._addToInQueue(function() {
                self.log.info('Ingesting next message from ' + pluginName + ' : ' + data.id + ' : ' + data.replyId);
...
Connector.prototype._messageReceived = function (pluginName, data) {
    ...
    this.db.findOne(query).exec()
        .then(function (result) {
            if (!result) {
                self.log.log(pluginName + ' message id ' + data.id + ' not found');
                data.o = 1;
                ...
                return Q.fcall(function () {
                    ...
                    .then(function (record) {
                        return self._sendUpdates(record, oData, replyTo);
                    });
            } else {
 ...
 */
Connector.prototype._sendUpdates = function (record, payload, replyRecord) {
    var work = [];

    for (var pluginName in this.plugins) {
        work.push(this._sendUpdate(pluginName, record, payload, replyRecord));
    }
 ...
 */
Connector.prototype._sendUpdate = function (pluginName, record, data, replyRecord) {
    var self = this;
